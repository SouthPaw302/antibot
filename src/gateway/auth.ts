...
export function resolveGatewayAuth(params: {
  ...
  const token =
    authConfig.token ??
    env.ANTIBOT_GATEWAY_TOKEN ??
    env.OPENCLAW_GATEWAY_TOKEN ??
    undefined;
  const password =
    authConfig.password ??
    env.ANTIBOT_GATEWAY_PASSWORD ??
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or ANTIBOT_GATEWAY_TOKEN / OPENCLAW_GATEWAY_TOKEN)",
    );
  }
